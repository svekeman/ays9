...
def test(job):
    ...
    try:
        cl.deleteServiceByName(name='kp', role='sshkey', repository=repo)
    except Exception as e:
        pass
    else:
    ...
    try:
        cl.deleteServiceByName(name='k1', role='sshkey', repository=repo)
    except Exception as e:
        failures.append("Wasn't able to remove service k1 even if won't break the minimum consumption required service cons1")

    try:
        cl.deleteServiceByName(name='k2', role='sshkey', repository=repo)
    except Exception as e:
        pass
    else:
