...
def test(job):
    ...
    try:
        ...
        if not any(u['name'] == user_name for u in account['data']['accountusers']):
        ...
        # execute blueprint for access right change
        ...
        if any(u['name'] == user_name and u['accesstype'] != accesstype for u in account['data']['accountusers']):
        ...
        if any(u['name'] == user_name for u in account['data']['accountusers']):
            failures.append(RESULT_FAILED % 'user not deleted from model data')
        ...
        limits = [account['data']['maxMemoryCapacity'], account['data']['maxCPUCapacity'],
                  account['data']['maxDiskCapacity'], account['data']['maxNumPublicIP']]
        if limits != expected_limits:
            failures.append(RESULT_FAILED % 'limits not updated in model data')
